...
async function getFilmBySlug(slug: string, userId?: string, isAdmin?: boolean) {
  ...
  // ✅ First, try to find the film by slug
  // We need to query by title (converted to slug)
  let query = supabase
    .from('content')
    .select(`*, profiles!inner ( full_name, bio, avatar_url )`)

  // ✅ For now, we need to fetch all and filter by slug
  // In production, you'd store a slug column in your database
  const { data: allContent } = await supabase
    .from('content')
    .select('*')
  
  // ✅ Find the content where the slug matches
  const film = allContent?.find(item => slugify(item.title) === slug)
  
  if (!film) return null
  
  // ✅ Now fetch the full data with the found ID
  ...
  if (!isAdmin) {
    if (userId) {
      ...
      if (filmCheck && filmCheck.creator_id !== userId) {
        filmQuery = filmQuery.eq('status', 'approved')
      }
    } else {
      filmQuery = filmQuery.eq('status', 'approved')
    }
  ...
  const { data, error } = await filmQuery.single()
